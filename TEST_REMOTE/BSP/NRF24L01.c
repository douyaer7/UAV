...
const u8 TX_ADDRESS[TX_ADR_WIDTH]={0x34,0xc3,0x10,0x10,0x10}; //发送地址
const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x34,0xc3,0x10,0x10,0x10}; //发送地址
...
{	
	...
	while(ReadIRQData(GPIO_Pin_15)!=0);
			sta=NRF_Read_Reg(STATUS);  //	   
			NRF_Write_Reg(WRITE_REG_NRF+STATUS,sta); //
